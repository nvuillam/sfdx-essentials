...
export default class ExecuteFilter extends Command {
  ...
  async run() {
    ...
    // Read package.xml files and build full list of items
    var allPackageXmlFilesTypes = {};
    var self = this
      ...
      parser.parseString(data, function (err2, result) {
        ...
        packageXmlMetadatasTypeLs.forEach(function (typePkg) {
          ...
          if (allPackageXmlFilesTypes[nameKey] != null && typePkg.members != null) {
            allPackageXmlFilesTypes[nameKey] = allPackageXmlFilesTypes[nameKey].concat(typePkg.members)
          }
          else if (typePkg.members != null) {
            allPackageXmlFilesTypes[nameKey] = typePkg.members
          }
        })
    ...
    console.log(`Package.xml files concatenation results :\n` + self.util.inspect(allPackageXmlFilesTypes, false, null))
  }
  ...
  getMetadataTypeDescription(md_type) {
    var desc = describeMetadataTypes()[md_type]
    return desc
  }
  ...
  filterSObjectFile(parsedObjectFile, objectName, objectContentToKeep) {
    const objectFilteringProperties = describeObjectFilteringProperties()
    var self = this
