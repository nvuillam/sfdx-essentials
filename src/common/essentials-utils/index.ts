...
class EssentialsUtils {
  ...
  public static async appendPackageXmlFilesContent(
    packageXmlFileList,
    { failIfError = false, ignoreDuplicateTypes = [], outputXmlFile = null, logFlag = false }
  ) {
    ...
    for (const packageXmlFile of packageXmlFileList) {
      ...
      try {
        ...
        throw new Error("Unable to parse package Xml file " + packageXmlFile);
      }
  ...
  public static async removePackageXmlFilesContent(
    ...
    try {
      ...
      throw new Error("Unable to parse package Xml file " + removePackageXmlFile);
    }
